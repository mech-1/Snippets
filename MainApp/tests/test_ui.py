...
def test_navigation_buttons_for_unauthorized_user(browser, live_server):
    ...
    # Нажимаем на гамбургер, если он существует
    try:
        hamburger = browser.find_element(By.CLASS_NAME, "navbar-toggler")
        hamburger.click()
    except:
        pass  # если гамбургера нет — значит меню уже видно

    # Проверяем, что кнопка "Посмотреть сниппеты" доступна
    # view_snippets_button1 = WebDriverWait(browser, 10).until(
    #     EC.visibility_of_element_located((By.XPATH, "//a[contains(text(), 'Посмотреть сниппеты')]"))
    # )
    view_snippets_button = browser.find_element(By.XPATH, "//a[contains(text(), 'Посмотреть сниппеты')]")
    # doesn't work is_displayed while
...
def test_navigation_buttons_for_authorized_user(browser, live_server):
    ...
    """
    # get user from fixture in conftest
